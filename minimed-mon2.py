...
#    24/01/2015 - Porting to UIFlow 2.x Micropython firmware
#
#  TODO:
#
...
class M5Msgbox:
   def __init__(self,btns_list, x, y, w, h, parent):
      ...
      self.x = x
      self.y = y
      self.w = w if w != None else 200
      self.h = h if h != None else 30
      self.rect_h = M5.Widgets.Rectangle(self.x,self.y,self.w,self.h,0xFFFFFF,0xFFFFFF)
      self.text_h = M5.Widgets.Label("msg",self.x,self.y,font=M5.Widgets.FONTS.DejaVu24)
      self.btns_list = btns_list
   def setText(self, text):
      self.text_h.setText(text=text)
   def delete(self):
      self.rect_h.setVisible(False)
      self.text_h.setVisible(False)
...
def do_access_point(ntpserver,timezone,proxyport):
   ...
   nvs0 = nvs("mmmon")
   nvs0.set_str('wifissid',  wifissid)
   time.sleep_ms(100)
   nvs0.set_str('wifipass',  wifipass)
   time.sleep_ms(100)
   nvs0.set_str('ntpserver', ntpserver)
   time.sleep_ms(100)
   nvs0.set_str('timezone',  timezone)
   time.sleep_ms(100)
   nvs0.set_str('proxyaddr', proxyaddr)
   time.sleep_ms(100)
   nvs0.set_str('proxyport', proxyport)
   time.sleep_ms(100)
...
def read_config():
   ...
   nvs0 = nvs("mmmon")
   try:
      wifissid  = nvs0.get_str('wifissid')
   except:
      wifissid = None
   try:
      wifipass  = nvs0.get_str('wifipass')
   except:
      wifipass = None
   try:
      ntpserver = nvs0.get_str('ntpserver')
   except:
      ntpserver = None
   try:
      timezone  = nvs0.get_str('timezone')
   except:
      timezone = None
   try:
      proxyaddr = nvs0.get_str('proxyaddr')
   except:
      proxyaddr = None
   try:
      proxyport = nvs0.get_str('proxyport')
   except:
      proxyport = None
...
def buttonA_wasPressed():
  ...
  pass
M5.BtnA.wasPressed(buttonA_wasPressed)
...
def buttonB_wasPressed():
  ...
  pass
M5.BtnB.wasPressed(buttonB_wasPressed)
...
def buttonC_wasPressed():
  ...
  pass
M5.BtnC.wasPressed(buttonC_wasPressed)
'''
...
def align_text(label,pos,y):
    if pos=="left":
        label.setCursor(x=0,y=y)
    elif pos=="center":
        #label.setCursor(x=160-int(label.get_width()/2),y=y) # FIXME
        pass
    elif pos=="right":
        #label.setCursor(x=320-label.get_width(),y=y) # FIXME
        pass


def time_delta(tm,timezone):
   if tm != None:
      datetime = time.localtime()
      print(tm)
      print(datetime)
      delta_min  = datetime[4] - tm[4]
      if delta_min < 0:
         delta_min += 60
      print("delta_min: "+str(delta_min))
      #delta_hour = datetime[3] - (tm[3]+int(timezone)+dstDelta)
      delta_hour = datetime[3] - tm[3]
      if delta_hour < 0:
         delta_hour += 24
...
def time_to_calib_progress(cfs,ttc,sst,cst):
   ...
   if (ttc == 255 or cst == "UNKNOWN") and not cfs: # unknown
      ...
      imageDrop.setImage("res/mm_drop_unk.png")
      imageDrop.setCursor(106, 8)
      #lcd.arc(centerX, centerY, radius, thick, 0, endposfull,0x00cccc,0x00cccc) # FIXME
   elif ttc >= 12: 
      ...
      if sst == "CALIBRATION_REQUIRED":
         # no circle, red drop
         imageDrop.setImage("res/mm_drop_red.png")
         imageDrop.setCursor(100, 0)
      else:
...
def handle_ntpsync(ntpserver, timezone):
   print("handle_ntpsync()")
   # Periodic timer: sync time via NTP
   try:
      tz = int(timezone)
      if tz > 0:
         time.timezone("GMT-%d" % (tz))
      else:
         time.timezone("GMT%d" % abs(tz))
      ntptime.host = ntpserver
      #tdelta = (int(timezone)+dstDelta)*3600
      #ts = ntptime.time() + tdelta
      ntptime.settime()
      datetime = time.localtime()
      print("time: %02d:%02d (tz:%d, dd:%d)" % (datetime[3],datetime[4],int(timezone),dstDelta))
      return True
   except:
      return False
...
def handle_timeupdate(timezone):
   print("handle_timeupdate()")
   try:
      # Update display time
      datetime = time.localtime()
      timeStr = ("%02d:%02d") % (datetime[3],datetime[4])
      labelTime.setText(timeStr)
      align_text(labelTime,"right",0)
      labelLastData.setText(time_delta(lastUpdateTm,timezone))
...
def handle_pumpdataupdate(proxyaddr, proxyport):
   ...
   try:
      r = requests.request(method='GET', url=proxy_url, headers={}, timeout=5)
   except OSError:
      r = None
   ...
   if r != None and r.status_code == 200 and r.json() != "":
      ...
      try:
         ...
         if haveData:
            print("we have valid data")
            imageBattery.setImage("res/mm_batt"+str(r.json()["pumpBatteryLevelPercent"])+".png")
            ...
            imageReservoir.setImage("res/mm_tank_unk.png")
            imageSage.setImage("res/mm_sage_unk.png")
            labelSage.setText("")
         
         if r.json()["conduitSensorInRange"]:
            imageSensorConn.setImage("res/mm_sensor_connection_ok.png")
         else:
            imageSensorConn.setImage("res/mm_sensor_connection_nok.png")
         ...
         if not haveData or r.json()["therapyAlgorithmState"]["autoModeShieldState"] == "FEATURE_OFF":
            ...
            imageShield.setImage("res/mm_shield_"+r.json()["lastSGTrend"].lower()+".png")
         ...
         if haveData:
            labelActInsValue.setText(str(round(r.json()["activeInsulin"]["amount"],1))+" U")
         else:
            labelActInsValue.setText("-- U")
      ...
      try:
         ...
         if systemStatus == "NO_ERROR_MESSAGE":
            ...
            lastStatusMsg.setText(systemStatus.replace("_"," "))
      except:
         if lastStatusMsg != None:
      ...
      try:
         ...
         imageBanner.setImage("res/mm_banner_"+pumpBanner.lower()+".png")
      ...
      '''
      try:
         labelAboveTargetValue.setText(str(r.json()["aboveHyperLimit"])+" %")
         labelInTargetValue.setText(str(r.json()["timeInRange"])+" %")
         labelBelowTargetValue.setText(str(r.json()["belowHypoLimit"])+" %")
         labelAverageSgValue.setText(str(r.json()["averageSG"])+" mg/dl")
      except:
         pass
      '''
...
def ttimer5(t):
   ...
   lastErrorMsg.setText("ERROR: urequests is stuck, reset device")
...
def setup():
   ...
   global imageBattery
   global imageReservoir
   global imageSensorConn
   global imageDrop
   global imageSage
   global imageShield
   global imageBanner

   global labelBglValue
   global labelBglUnit
   global labelActInsValue
   global labelActIns
   global labelTime
   ...
   M5.Lcd.setTextColor(M5.Lcd.COLOR.WHITE)
   M5.Lcd.setCursor(0,0)
   M5.Lcd.print("Minimed Mon, ver %s" % (VERSION))
   ...
   imageBattery     = M5.Widgets.Image("res/mm_batt_unk.png", x=6, y=0, parent=scr1)
   imageReservoir   = M5.Widgets.Image("res/mm_tank_unk.png", x=40, y=0, parent=scr1)
   imageSensorConn  = M5.Widgets.Image("res/mm_sensor_connection_nok.png", x=68, y=0, parent=scr1)
...
def init():
   msgbox = None
   while handle_ntpsync(ntpserver, timezone) == False:
      time.sleep_ms(1000)
      if msgbox == None:
         msgbox = M5Msgbox(btns_list=None, x=0, y=0, w=None, h=None, parent=scr1)
         msgbox.setText("Trying to synch time and date ...")
   if msgbox != None:
      msgbox.delete()
   ...
   timer0 = Timer(0)
   timer0.init(mode=Timer.PERIODIC, period=int(TIMER0_PERIOD_S*1000), callback=ttimer0)

   # Timer 1: 10 sec (periodic) // timeupdate
   TIMER1_PERIOD_S = 10
   timer1 = Timer(1)
   timer1.init(mode=Timer.PERIODIC, period=int(TIMER1_PERIOD_S*1000), callback=ttimer1)

   # Timer 2: 60 sec (periodic) // pumpdataupdate
   TIMER2_PERIOD_S = 60
   timer2 = Timer(2)
   timer2.init(mode=Timer.PERIODIC, period=int(TIMER2_PERIOD_S*1000), callback=ttimer2)

   # Timer 3: 0.2 sec (periodic) // touchevent
   TIMER3_PERIOD_S = 0.2
   timer3 = Timer(3)
   #timer3.init(mode=Timer.PERIODIC, period=int(TIMER3_PERIOD_S*1000), callback=ttimer3)
